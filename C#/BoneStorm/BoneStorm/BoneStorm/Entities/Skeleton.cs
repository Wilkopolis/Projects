...
namespace BoneStorm
{
    class Skeleton : Character
    {
        ...
        public Segment[] GetSides(Player player)
        {
            ...
            //BoneStormClient.Line1 = new Segment(Vector2.Zero, Vector2.Zero);
            //BoneStormClient.Line2 = new Segment(Vector2.Zero, Vector2.Zero);
            //BoneStormClient.Line3 = new Segment(Vector2.Zero, Vector2.Zero);
            //BoneStormClient.Line4 = new Segment(Vector2.Zero, Vector2.Zero);
            //BoneStormClient.Line1 = new Segment(new Vector2(Pos.X + Origin.X, Pos.Y + Origin.Y), new Vector2(Pos.X + Origin.X, Pos.Y - Origin.Y));
            //BoneStormClient.Line2 = new Segment(new Vector2(Pos.X - Origin.X, Pos.Y - Origin.Y), new Vector2(Pos.X - Origin.X, Pos.Y + Origin.Y));
            //BoneStormClient.Line3 = new Segment(new Vector2(Pos.X + Origin.X, Pos.Y - Origin.Y), new Vector2(Pos.X - Origin.X, Pos.Y - Origin.Y));
            //BoneStormClient.Line4 = new Segment(new Vector2(Pos.X - Origin.X, Pos.Y + Origin.Y), new Vector2(Pos.X + Origin.X, Pos.Y + Origin.Y));
            var pc = player.Pos;


            if (pc.X > myRight)
                results.Add(new Segment(new Vector2(Pos.X + Origin.X, Pos.Y + Origin.Y), new Vector2(Pos.X + Origin.X, Pos.Y - Origin.Y)));
            else if (pc.X < myLeft)
                results.Add(new Segment(new Vector2(Pos.X - Origin.X, Pos.Y - Origin.Y), new Vector2(Pos.X - Origin.X, Pos.Y + Origin.Y)));

            if (pc.Y < myTop)
                results.Add(new Segment(new Vector2(Pos.X + Origin.X, Pos.Y - Origin.Y), new Vector2(Pos.X - Origin.X, Pos.Y - Origin.Y)));
            else if (pc.Y > myBottom)
                results.Add(new Segment(new Vector2(Pos.X - Origin.X, Pos.Y + Origin.Y), new Vector2(Pos.X + Origin.X, Pos.Y + Origin.Y)));
    ...
    class DootDoot : Skeleton
    {
        ...
        new void Animate()
        {
            ...
            if (DootFrameCount == DootFrame)
                BoneStormClient.SpawnDoot(DootType);

...
namespace BoneStorm
{
    ...
    class Player : Character
    {
        ...
        public void Update(List<Platform> platforms, List<Rectangle> killZones, List<PickUp> pickUps)
        {
            ...
            foreach (var r in killZones)
            {
                ...
                if ((myRight >= r.Left && myLeft <= r.Right) && (myTop <= r.Bottom && myBottom >= r.Top))
                {
                    //rip
                    HP = 0;
                    BoneStormClient.gameStarted = false;
                }
            }
        ...
        public void Shoot()
        {
            ...
            CanShoot = false;
            cooldownFrames = 0;
            if (BoneStormClient.Shoot())
                BoneStormClient.CheckDead();
        }
        ...
        public bool CanSpecial()
        {
            ...
            {
                case Special.DPS:
                    return Effects.Count(e => e.Type == Buffs.DPS) <= 0;                       

                case Special.PLATFORM:
                    return specialCooldownFrames >= specialCooldown && boneCount >= specialCost;
        ...
        {
            if (boneCount >= specialCost)
            {
                boneCount -= specialCost;
                ...
                {
                    case Special.PLATFORM:
                        BoneStormClient.ApplySpecial(Special);
                        break;

                    case Special.DPS:
                        shootCooldown = DPSAttackSpeed;
                        Effects.Add(new Effect(Buffs.DPS, 200));
                        break;
                }
            }
        }

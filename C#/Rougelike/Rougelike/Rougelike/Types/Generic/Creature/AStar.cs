...
namespace Rougelike.Types
{
    ...
    {
        private static void AStarHorseshit(Enemy Enemy, Player kevin, Room Room, int Distance)
        {
            ...
            current.X = (int)Enemy.Position.X;
            current.Y = (int)Enemy.Position.Y;
            ...
            if (Math.Abs(kevin.Position.X - current.X) + Math.Abs(kevin.Position.Y - current.Y) == Distance)
            {
                ...
                {
                    Enemy.Movements.AddFirst(new Movement((int)current.X, (int)current.Y, mod));
                    current = current.previous;
                }
            }
            else
            {
                AStarHorseshit(Enemy, kevin, Room, ++Distance);
            }
        }

        private static AStarNode FindLowestFScore(LinkedList<AStarNode> list, Player kevin)
        {
        ...
        private static int CalcFScore(AStarNode node, Player kevin)
        {
            //Calc hscore

...
function lightLine (p, q, tiles) {
	...
	var err = (dx > dy ? dx : -dy) / 2;
 	...
 	while (true) {
 		...
 		if (tiles[y0][x0].entities.length > 0) {
 			tiles[y0][x0].entities.peek().visible = true;
 			
 			if (typeof tiles[y0][x0].entities.peek().discovered !== 'undefined') {
	 			if (bored && !tiles[y0][x0].entities.peek().discovered) {
	 				tiles[y0][x0].entities.peek().discovered = true;
	 				log.add("A " + tiles[y0][x0].entities.peek().name + " comes into view.")
	 				bored = false;
	 			}
 			}
 		}
...
function traceLine (p, q, tiles) {
	...
	var err = (dx > dy ? dx : -dy) / 2;
 	...
 	while (true) {
 		...
 		if (tiles[y0][x0].entities.length > 0 && typeof tiles[y0][x0].entities.peek().discovered !== 'undefined') {
 			if (bored && !tiles[y0][x0].entities.peek().discovered) {
 				tiles[y0][x0].entities.peek().discovered = true;
 			}
 		}

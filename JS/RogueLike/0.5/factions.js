...
var PRICES = {ant:2, dog:5, monkey:7, gorilla:15,
			  ...
			  wanderer:2, marauder:5, techie:8, WareChief:18,
			  goo:5
			 };
...
function faction_survivor(pos) {
	...
	this.spendWealth = function() {
		...
		var valueArray = [];
		valueArray.push({name:NAME_WANDERER, value: 14 - RoomRatio - WPT - 2 * UnitTotal});
		valueArray.push({name:NAME_MARAUDER, value: .1 + RoomRatio - WPT * .4 + .6 * UnitTotal});
		valueArray.push({name:NAME_TECHIE, value: RoomRatio - WPT * .4 + .6 * UnitTotal});
		valueArray.push({name:NAME_WAR_BOSS, value: 2 + RoomRatio - WPT * .5 + (UnitTotal - warBossCount * 6)});
...
function MakeEnemy(type, pos, statuses) {
	switch(type) {
		...
		return new Enemy(pos, type, 10, 1, 5, 'a', FACTION_ANIMAL, statuses);
		break;
		case NAME_DOG:
		factions[FACTION_ANIMAL].dogCount++;
		return new Enemy(pos, type, 10, 1, 5, 'd', FACTION_ANIMAL, statuses);
		break;
		case NAME_MONKEY:
		factions[FACTION_ANIMAL].monkeyCount++;
		return new Enemy(pos, type, 10, 1, 5, 'm', FACTION_ANIMAL, statuses);
		break;
		case NAME_GORILLA:
		factions[FACTION_ANIMAL].gorillaCount++;
		return new Enemy(pos, type, 10, 1, 5, 'G', FACTION_ANIMAL, statuses);
		break;
		case NAME_WANDERER:
		factions[FACTION_SURVIVOR].wandererCount++;
		return new Enemy(pos, type, 10, 1, 5, 'w', FACTION_SURVIVOR, statuses);
		break;
		case NAME_MARAUDER:
		factions[FACTION_SURVIVOR].marauderCount++;
		return new Enemy(pos, type, 10, 1, 5, 'M', FACTION_SURVIVOR, statuses);
		break;
		case NAME_TECHIE:
		factions[FACTION_SURVIVOR].techieCount++;
		return new Enemy(pos, type, 10, 1, 5, 'C', FACTION_SURVIVOR, statuses);
		break;
		case NAME_WAR_BOSS:

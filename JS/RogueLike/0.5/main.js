...
function selectInventoryItem(shift) {
	var item = player.inventory[inventoryCursorPos];
	switch (item.slot) {
		case SLOT_WIELDABLE:
...
function Character () {
	...
	this.applyStatuses = function() {
		switch(this.getEffectLevel(SKILL_REGEN)) {
			case 0:break;
			case 1:
	...
	this.getDmg = function() {
		...
		if (this.offHand != ITEM_NONE && typeof this.offHand.dmg !== "undefined")
			weapons.push(player.offHand);

		switch (weapons.length) {
			case 0:break;
			case 1:result += weapons[0].dmg; break;
			case 2:result += .5*weapons[0].dmg; result += .5*weapons[1].dmg; break;
			default:break;
		}
	...
	this.addXp = function(amount) {
		if (this.xp < this.nextLevelXp && this.xp + amount >= this.nextLevelXp) {
			this.skillPoints += 5;
...
var SKILL_REGEN = 'Regen';
function Skills (occupation) {
	// default skills
	this.defaultSkills = [];
	this.defaultSkills.push({name:SKILL_REGEN, lvl:0, maxLvl:5});

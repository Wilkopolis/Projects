...
var OBJ_GENES = 'genes';
var MAIN_OBJECTIVES = [{type:OBJ_GENERATORS, sizes:[1,2,3,4]}, {type:OBJ_GENES, sizes:[1,2,3,4]}];
var OBJ_AUX_GENERATOR = 'aux_generators';
var OBJ_MASTERMIND = 'mastermind';
var OBJ_CITY_OS = 'deliver';
var SIDE_OBJECTIVES = [{type:OBJ_MASTERMIND, sizes:[1]}, {type:OBJ_CITY_OS, sizes:[1,2,3,4]}];
...
function generator (id, room_segment) {
	...
	this.startUp = function() {
		...
		var enemy = MakeNPC(NAME_NIGHT_GAURD, {x:0, y:0}, [{type:STATUS_STUNNED, ticksRemaining:1, unique:true}]);
		// pick an amount
	...
	this.getTitleString = function() {
		...
		else if (this.finished && this.failed)
			return "Generator " + this.id + " was started up.";
		else if (this.finished && !this.failed)
			return "Generator " + this.id + " was successfully powered on.";
		else
			return "Generator " + this.id + " needs to be started up.";
	}
}
		...
		for (var i = MASTERMIND_SOLUTION_LENGTH - 1; i >= 0; i--) {
			...
			if (!correct) {
				...
				for (var j = MOVE_OPTIONS.length - 1; j >= 0; j--) {
					var tilePos = {x:this.pieces[i].x + MOVE_OPTIONS[j].x, y:this.pieces[i].y + MOVE_OPTIONS[j].y}
				...
				var enemy = MakeNPC(NAME_NIGHT_GAURD, {x:enemyPos.x, y:enemyPos.y}, [{type:STATUS_STUNNED, ticksRemaining:1, unique:true}]);
...
function item_os_disk () {
	...
	this.getHTMLDescription = function() {
		...
		element.innerHTML = this.char + " - The code that runs this place.";

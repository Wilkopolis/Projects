...
function getPathAStar(character, destination) {
	...
	while (openSet.length > 0) {
		...
		for (var i = MOVE_OPTIONS.length - 1; i >= 0; i--) {
			...
			if (!closedSet.setContains(newNode) && !dungeon.tiles[newNode.y][newNode.x].getPermanentSolid(character.faction) && !openSet.setContains(newNode))
				openSet.setSmartInsert(newNode);
		}
	}
	var result = []
...
function autoExplore() {
	...
	while (bored) {
		...
		while(!unvisitedRoomSegment.visited && bored) {
			// path to it
			var offset = path.pop();

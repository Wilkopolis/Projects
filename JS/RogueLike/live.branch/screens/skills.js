	...
	if (skillSelecting) {
		...
		if (skill.requirementsMet() && !skill.purchased) {
	   		...
	   		switch(skill.name) {
	   			...
	   			case SKILL_MAN_OF_ARMS_I: player.armCount++; break;
	   			case SKILL_MAN_OF_ARMS_II: player.armCount++; break;
	   			case SKILL_MAN_OF_ARMS_III: player.armCount++; break;
	   			case SKILL_SPARK_ARMOR: player.shields[SKILL_SPARK_ARMOR] = {capacity:player.willpower, currentStrength:player.willpower, ticksRemaining: -1}; break;
	   			case SKILL_BLINK_FREQUENCY: 
	   				player.skills.skillObject[SKILL_BLINK_I].cooldown = Math.round(player.skills.skillObject[SKILL_BLINK_I].cooldown / 3);
	   				player.skills.skillObject[SKILL_BLINK_II].cooldown = Math.round(player.skills.skillObject[SKILL_BLINK_II].cooldown / 3);
	   			break;
	   		}
	   		windows[SCREEN_GAME].redraw(gameTicks);
	   	}	    	

...
function Inventory () {
	...
	this.pickUp = function(item) {
		this.push(item);
		if (item.name == NAME_VAULT_KEY) {
			var keys = Object.keys(roomLookup);
			for (var i = keys.length - 1; i >= 0; i--) {
				var room = roomLookup[keys[i]];
				if (room.event == FLAVOR_VAULT) {
					room.unlockDoors();
					break;
				}
			}

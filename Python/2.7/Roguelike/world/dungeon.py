...
class Dungeon(object):
    ...
    def __init__(self):
        ...
        room_count = 0
        room_hash = 1
        # take some memory thats gonna be our map
        self.minimap = [[Room() for x in range(17)] for y in range(17)]

        starting_room_pos = Vector(len(self.minimap[0])/2, len(self.minimap)/2)

        # start at the center
        self.minimap[starting_room_pos.x][starting_room_pos.y].hash = room_hash
        room_hash += 1
        
        # starting at the center, navigate through the grid and build a map
        openSet = MapVectorSet([starting_room_pos]);
        free_rooms = []
        while room_count < room_total and len(openSet) > 0:
            # pick a random element from the openset
            currentPos = openSet.pop_weighted()
            if currentPos.distance >= 2:
                free_rooms.append(Room(room_hash, currentPos.x, currentPos.y))
        #    # get a weighted list of the shapes
        #    validShapes = getValidShapes(currentPos)
        #    # pick the room shape
        #    shape = validShapes.pickWeighted()
        #    # but start in a 1 x 1 room okay :)
        #    if roomCount == 0:
        #        shape = ROOM_SHAPES[0];
        #    # fill the grid given the room shape
        #    for i in range(0, len(shape.offsets.length)):
        #        # get the offset x/y pair
        #         offset = shape.offsets[i]
        #         newPos = {x: currentPos.x + offset.x, y: currentPos.y + offset.y}
        #        # fill the block at current pos + offset
            #    minimap[newPos.x][newPos.y].hash = room_hash
            #    roomLookup[hash] = Room(room_hash)
